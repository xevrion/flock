...
If the demo shows "Server not available", start the server locally first (`npx @flock-sdk/server`) and set `NEXT_PUBLIC_FLOCK_SERVER_URL=ws://localhost:8787` in `apps/docs/.env.local`.
